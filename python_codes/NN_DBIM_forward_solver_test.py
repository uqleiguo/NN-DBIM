...
import torch
import utils
import NN_DBIM_forward_solver_model
import time
...
def evaluate(model, test_set, Ez_inc, plot_flag):
    ...
    return ep, ep_gt, e_tot, e_tot_gt
...
if __name__ == '__main__':
    ...
    model = NN_DBIM_forward_solver_model.UNet(out_channels=2, channel_num=channels, e_inc=Ez_inc_new).to(device)

    model_idx = np.arange(5000, 5050, 50)

    model_name = '../models/NN_DBIM_forward_solver.pkl'

    model.load_state_dict(torch.load(model_name, map_location='cpu'))
    ep, ep_gt, e_tot, e_tot_gt = evaluate(model, test_dataset, FD_Ez_inc, plot_flag=1)
